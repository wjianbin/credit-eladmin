...
public class CustomerCardpipyTask {
	...
	public void runNewData() throws Exception {
		...
		try {
			BusCustomerCardpipyQueryCriteria criteria = new BusCustomerCardpipyQueryCriteria();
			...
			List<BusCustomerCardpipyDto> all = busCustomerCardpipyService.queryAll(criteria);
			// 在固定目录下生成报文并打包
			busCustomerCardpipyService.downloadCreditFile(all);
			// 更新任务状态
	...
	private BusTaskLog init(String businessDate) throws Exception {
		...
		resources.setTaskdescribe(TaskConstants.TASK_NAME_NEW_CUSTOMER_CARDPIPY_DECRB);
		resources.setTaskname(TaskConstants.TASK_NAME_NEW_CUSTOMER_CARDPIPY);
		return resources;
	}
