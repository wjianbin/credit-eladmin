...
public class DelCustomerInfoTask {
	...
	public void runNewData() throws Exception {
		...
		try {
			BusDelCustomerInfoQueryCriteria criteria = new BusDelCustomerInfoQueryCriteria();
			...
			// 在固定目录下生成报文并打包
			busDelCustomerInfoService.downloadCreditFile(all);
			// 更新任务状态
	...
	private BusTaskLog init(String businessDate) throws Exception {
		...
		resources.setTaskdescribe(TaskConstants.TASK_NAME_DEL_CUSTOMERINFO_DECRB);
		resources.setTaskname(TaskConstants.TASK_NAME_DEL_CUSTOMERINFO);
		return resources;
	}
