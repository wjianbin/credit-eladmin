...
public class CustomerRelationshipTask {
	...
	public void runNewData() throws Exception {
		...
		try {
			BusCustomerRelationshipQueryCriteria criteria = new BusCustomerRelationshipQueryCriteria();
			...
			List<BusCustomerRelationshipDto> all = busCustomerRelationshipService.queryAll(criteria);
			// 在固定目录下生成报文并打包
			busCustomerRelationshipService.downloadCreditFile(all);
			// 更新任务状态
	...
	private BusTaskLog init(String businessDate) throws Exception {
		...
		resources.setTaskdescribe(TaskConstants.TASK_NAME_NEW_CUSTOMER_RELATIONSHIP_DECRB);
		resources.setTaskname(TaskConstants.TASK_NAME_NEW_CUSTOMER_RELATIONSHIP);
		return resources;
	}
