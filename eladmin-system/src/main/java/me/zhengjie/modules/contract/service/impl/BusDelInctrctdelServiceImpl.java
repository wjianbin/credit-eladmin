...
public class BusDelInctrctdelServiceImpl implements BusDelInctrctdelService {
    ...
    @Override
    public void downloadCreditFile(List<BusDelInctrctdelDto> all, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (all == null) {
            throw new BadRequestException("请选择数据");
        }
        try {
            File file = new File(CreditInfoUtil.downloadDelInctrctdelFile(all, TaskConstants.Task_Bus_Del_Inctrctdel+ DateHelper.getCurrentTimeNoSLong(),
                    TaskConstants.Bus_Del_Inctrctdel));
    ...
    @Override
    public void downloadCreditFile(List<BusDelInctrctdelDto> all) throws Exception {
        if (all == null) {
            throw new BadRequestException("请选择数据");
        }
        try {
