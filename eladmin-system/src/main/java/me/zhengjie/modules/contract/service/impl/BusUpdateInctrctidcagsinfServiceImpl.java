...
public class BusUpdateInctrctidcagsinfServiceImpl implements BusUpdateInctrctidcagsinfService {
    ...
    @Override
    public void downloadCreditFile(List<BusUpdateInctrctidcagsinfDto> all, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (all == null) {
            throw new BadRequestException("请选择数据");
        }
        try {
            File file = new File(CreditInfoUtil.downloadUpdateInctrctidcagsinfFile(all, TaskConstants.Task_Bus_Update_Inctrctidcagsinf+ DateHelper.getCurrentTimeNoSLong(),
                    TaskConstants.Bus_Update_Inctrctidcagsinf));
    ...
    @Override
    public void downloadCreditFile(List<BusUpdateInctrctidcagsinfDto> all) throws Exception {
        if (all == null) {
            throw new BadRequestException("请选择数据");
        }
        try {
